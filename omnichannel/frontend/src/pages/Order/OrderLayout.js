...
import OrderSubMenu from './OrderSubMenu';
import { usePageTitle } from '../../context/HeaderContext';

export default function OrderLayout() {
  const [isCollapsed, setIsCollapsed] = useState(false);
  usePageTitle('ORDER & BILLING');

  return (
    <div className="p-3 sm:p-4 md:p-6 min-h-screen bg-gray-50 dark:bg-[#0f172a] transition-colors duration-200 pb-20 md:pb-6">
      <p className="text-gray-500 dark:text-slate-400 mb-4 md:mb-6 pl-1 text-xs sm:text-sm">Kelola langganan dan pembelian tambahan Anda.</p>
      ...
      <div className="flex flex-col md:flex-row gap-3 sm:gap-4 md:gap-6 items-start">
        ...
        <aside className={`w-full transition-all duration-300 ${isCollapsed ? 'w-full md:w-16' : 'w-full md:w-52'} flex-shrink-0 md:sticky md:top-6 relative`}>
           ...
           <OrderSubMenu isCollapsed={isCollapsed} />
        </aside>
