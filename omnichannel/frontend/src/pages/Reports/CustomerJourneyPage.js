...
import {
    ...
    Heart, AlertCircle, ChevronRight, ChevronLeft, ArrowLeft, Calendar, Filter, RefreshCw,
...
export default function CustomerJourneyPage() {
    ...
    return (
        <div className="flex flex-col md:flex-row h-[calc(100vh-64px)] overflow-hidden">
            {/* Left Panel - Contact List (hidden on mobile when contact is selected) */}
            <div className={`${selectedContact ? 'hidden md:flex' : 'flex'} w-full md:w-80 border-r border-gray-200 bg-gray-50 flex-col shrink-0 h-full`}>
                <div className="p-3.5 sm:p-4 border-b bg-white">
            ...
            <div className={`${selectedContact ? 'flex' : 'hidden md:flex'} flex-1 overflow-y-auto flex-col h-full bg-white`}>
                {selectedContact && (
                    <div className="md:hidden p-3 bg-white border-b border-gray-100 flex items-center justify-between">
                        ...
                        >
                            <ChevronLeft className="w-4 h-4" /> Kembali ke Kontak
                        </button>
                        <span className="text-xs font-medium text-gray-500 truncate max-w-[160px]">{selectedContact.name}</span>
                    </div>
                )}
                {!selectedContact ? (
                    <div className="flex items-center justify-center h-full text-gray-400 p-6">
                        <div className="text-center">
                            ...
                            <p className="text-base sm:text-lg">Select a contact to view their journey</p>
                        </div>
                    </div>
                ) : (
                        ...
                        <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 shadow-sm">
                            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                                <div className="flex items-start gap-3 sm:gap-4 w-full sm:w-auto">
                                    <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-xl sm:text-2xl font-bold shrink-0">
                                        {selectedContact.name?.charAt(0)?.toUpperCase() || '?'}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h2 className="text-lg sm:text-xl font-bold text-gray-900 truncate">{selectedContact.name}</h2>
                                        <p className="text-xs sm:text-sm text-gray-500">{selectedContact.phone_number}</p>
                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-gray-500">
                                            {selectedContact.email && <span>Email: {selectedContact.email}</span>}
                                            {selectedContact.lifetime_value > 0 && (
                                                <span className="text-green-600 font-medium">
                                                    LTV: Rp {selectedContact.lifetime_value.toLocaleString()}
                                                </span>
                                            )}
                                        </div>
                                    </div>
