    ...
    return (
        <div className="p-3.5 sm:p-6 md:p-8 h-full overflow-y-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-3 sm:gap-4">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900">General Overview</h2>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full md:w-auto">
                    <DateRangeFilter startDate={startDate} endDate={endDate} onChange={(s, e) => { setStartDate(s); setEndDate(e); }} />
                    <button
                        onClick={handleExport}
                        disabled={downloading}
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 font-medium text-sm transition-colors disabled:opacity-50 shrink-0"
                    >
            ...
            <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-100 shadow-sm mb-8">
                <h3 className="font-bold text-gray-800 mb-4 sm:mb-6 text-base sm:text-lg">Volume Percakapan</h3>
