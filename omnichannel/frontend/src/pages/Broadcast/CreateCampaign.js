...
const StepIndicator = ({ current, step, title }) => {
    ...
    return (
        <div className="flex items-center flex-1 last:flex-none">
            <div className={`w-7 h-7 sm:w-10 sm:h-10 text-xs sm:text-base rounded-full flex items-center justify-center font-bold transition-all duration-500 shadow-sm shrink-0 ${isActive ? 'bg-indigo-600 text-white shadow-indigo-500/30 scale-105 sm:scale-110 ring-2 sm:ring-4 ring-indigo-50 dark:ring-indigo-900/30' : isCompleted ? 'bg-green-500 text-white' : 'bg-gray-100 dark:bg-slate-800 text-gray-400 dark:text-slate-500'}`}>
                {isCompleted ? <CheckCircle className="w-3.5 h-3.5 sm:w-5 sm:h-5" /> : step}
            </div>
            <span className={`ml-1.5 sm:ml-3 text-xs sm:text-sm font-bold transition-colors duration-300 ${isActive ? 'inline-block text-indigo-600 dark:text-indigo-400' : 'hidden sm:inline-block ' + (isCompleted ? 'text-green-600 dark:text-green-500' : 'text-gray-400 dark:text-slate-500')}`}>{title}</span>
            {step < 3 && <div className={`flex-1 h-1 sm:h-1.5 mx-1.5 sm:mx-4 rounded-full transition-colors duration-500 ${isCompleted ? 'bg-green-500' : 'bg-gray-100 dark:bg-slate-800'}`}></div>}
        </div>
    );
};
...
export default function CreateCampaign() {
    ...
    return (
        <div className="flex h-full">
            <div className="flex-1 p-3.5 sm:p-6 md:p-8 overflow-y-auto custom-scrollbar">
                <div className="mb-6 sm:mb-8">
                    ...
                    {/* Auto-save indicator */}
                    {lastSaved && (
                        <div className="px-1 sm:px-4 flex items-center gap-2 text-xs text-gray-400">
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-4">
                                        <label className={`flex-1 p-3.5 sm:p-4 border rounded-xl cursor-pointer transition-all ${broadcastType === 'personal' ? 'bg-indigo-50 border-indigo-500 ring-1 ring-indigo-200' : 'hover:bg-gray-50'} ${isInvoiceMode ? 'opacity-60 pointer-events-none cursor-not-allowed bg-gray-50 border-gray-200' : ''}`}>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="block font-bold text-gray-800 text-sm sm:text-base">Personal (Contact)</span>
                                                    <span className="block text-xs text-gray-500">Send to individual numbers</span>
                                                </div>
                                            </div>
                                        </label>
                                        <label className={`flex-1 p-3.5 sm:p-4 border rounded-xl cursor-pointer transition-all ${broadcastType === 'group' ? 'bg-indigo-50 border-indigo-500 ring-1 ring-indigo-200' : 'hover:bg-gray-50'} ${isInvoiceMode ? 'opacity-60 pointer-events-none cursor-not-allowed bg-gray-50 border-gray-200' : ''}`}>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="block font-bold text-gray-800 text-sm sm:text-base">Community (Group)</span>
                                ...
                                {broadcastType === 'personal' && (
                                    <div>
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-3">
                                            <label className={`flex items-center gap-2 cursor-pointer border px-3 py-2.5 rounded-lg w-full sm:w-1/2 hover:bg-gray-50 ${senderType === 'rotator' ? 'bg-indigo-50 border-indigo-200' : ''}`}>
                                            ...
                                            <label className={`flex items-center gap-2 cursor-pointer border px-3 py-2.5 rounded-lg w-full sm:w-1/2 hover:bg-gray-50 ${senderType === 'device' ? 'bg-indigo-50 border-indigo-200' : ''}`}>
                ...
                {step === 2 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-3 pt-4">
                            <button onClick={() => setStep(1)} className="w-full sm:w-auto px-6 py-2.5 text-gray-600 dark:text-gray-400 font-bold hover:text-gray-900 dark:hover:text-white transition-colors">Back</button>
                            <button
                                onClick={() => setStep(3)}
                                disabled={(!message && !selectedMetaTemplate) || !isSpintaxValid}
                                className="w-full sm:w-auto px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-500/30 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                ...
                {step === 3 && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        {/* Header Summary Card */}
                        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 sm:p-5 rounded-xl border border-indigo-100 shadow-sm">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div>
                                    <h2 className="text-lg sm:text-xl font-bold text-gray-900">Ready to Launch</h2>
                                    <p className="text-gray-600 text-xs sm:text-sm mt-1">
                                        Sending <strong>"{campaignName}"</strong> via <strong>{isOfficialDevice ? 'Official API' : 'WhatsApp Web'}</strong>
                                    </p>
                                </div>
                                <div className="bg-white px-4 py-2 rounded-lg border border-indigo-100 shadow-sm text-center self-start sm:self-auto shrink-0">
                                    <div className="text-xl sm:text-2xl font-bold text-indigo-600">{targetCount}</div>
                            ...
                            {broadcastType === 'group' ? (
                                ...
                                <div className="space-y-3">
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                                        <button onClick={() => setTargetType('all')} className={`p-2 border rounded-lg text-xs sm:text-sm font-medium transition-all ${targetType === 'all' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold shadow-xs' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>All</button>
                                        <button onClick={() => setTargetType('device')} className={`p-2 border rounded-lg text-xs sm:text-sm font-medium transition-all ${targetType === 'device' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold shadow-xs' : 'bg-white text-gray-600 hover:bg-gray-50'}`}><Smartphone className="w-3.5 h-3.5 inline mr-1" /> Device</button>
                                        <button onClick={() => setTargetType('label')} className={`p-2 border rounded-lg text-xs sm:text-sm font-medium transition-all ${targetType === 'label' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold shadow-xs' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>Label</button>
                                        <button onClick={() => setTargetType('file')} className={`p-2 border rounded-lg text-xs sm:text-sm font-medium transition-all ${targetType === 'file' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold shadow-xs' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>File</button>
                                        <button onClick={() => setTargetType('segmented')} className={`p-2 border rounded-lg text-xs sm:text-sm font-medium transition-all col-span-2 sm:col-span-1 ${targetType === 'segmented' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold shadow-xs' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>Segmented</button>
                                    </div>
                                    {targetType === 'all' && (
                        ...
                        {/* Action Buttons */}
                        <div className="flex flex-col-reverse sm:flex-row justify-center gap-3 sm:gap-4 pt-6 sm:pt-8 border-t mt-6 sm:mt-8">
                            <button onClick={() => setStep(2)} className="w-full sm:w-auto px-6 py-3 border border-gray-200 dark:border-slate-700 rounded-xl text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-50 dark:hover:bg-slate-800 transition-all duration-200 hover:-translate-y-0.5 active:scale-95 text-center">Edit Content</button>
                            <button onClick={handleSubmit} disabled={loading || !isSpintaxValid} className="w-full sm:w-auto px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-500/30 flex items-center justify-center gap-2 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed text-center">
