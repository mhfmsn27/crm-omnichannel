...
import { ensurePipelineSchema } from '../pipelineController.js';
    ...
    try {
        ...
        const result = await pool.query(`
            SELECT
                ...
                COALESCE(SUM(c.value * COALESCE(c.deal_probability, 100) / 100.0), 0) as weighted_value,
        ...
        console.error('[SalesReport] getPipelineStats error:', e.message);
        // Fallback: in case deal_probability is still resolving, query safely without crashing
        try {
            const fallback = await pool.query(`
                SELECT
                    ...
                    p.name as pipeline_name,
                    COUNT(c.id) as deal_count,
                    COALESCE(SUM(COALESCE(c.value, 0)), 0) as total_value,
                    COALESCE(SUM(COALESCE(c.value, 0)), 0) as weighted_value,
                    COUNT(*) FILTER (WHERE c.status = 'resolved') as won_count,
                    COUNT(*) FILTER (WHERE c.status = 'closed') as lost_count
                FROM pipeline_stages ps
                JOIN pipelines p ON ps.pipeline_id = p.id
